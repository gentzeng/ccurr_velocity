...
\begin{algorithm*}[!h]%
	\DontPrintSemicolon
  \caption{Moved-coin-approach: Measurement $\protect\MCircM$ for $\mathtt{type}$ (\acs*{fifo}, \acs*{lifo}) for period $\perd$, look-back window $\wndw$.%
  }\label{algo:code_mcirc_mc}%
%
	\KwData{$\wndw_\Start$ \tcc*{Beginning of look-back window $w$}}%
	\KwData{$ \TxW$                                                                     \tcc*{Set of transactions in period $\wndw$}}%
	\KwData{$ \Out^{\selfchurn}$                                                        \tcc*{Set of self-churning outputs}}%
%	\KwResult{$\MCircM$                                                                 \tcc*{Money supply to be estimated in period $\wndw$ with window $\wndw$}}%
%
	$\MCircM \gets {0}$																	\tcc*{Money supply to be estimated for period $\perd$ with window $\wndw$}%
%
	\ForEach {$ t \in \TxW $%
%		                                                            \tcc*{Loop through each transaction $t$ in period $\wndw$}%
	                                                            }%
	{%
...
%	    	                                              \tcc*{Loop through each input $i$ of sorted input set $\InpSortedT$}%
	...
	return $\MCircM$                                                                    \tcc*{Result: Return money in circulation within window $\wndw$ as estimated for period $\perd$ for \ac*{mca}}%
\end{algorithm*}%
\else%
\fi%
