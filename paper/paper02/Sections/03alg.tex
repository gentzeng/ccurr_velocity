...
Let's first turn to the computation of \(\MSetCircLA[d]\) and \(\MSetCirc_{1}[d]\) respectivly. %
...
The computation of adjustment value \(\MSetCircLB[d]\) is more complex. %
...
For every transaction \(t\), \(\MCircAdj(t,w)\) loops through all outputs and establishes whether it reaches out of the new window \(\wndw\) formed by adding day \(d\) and eliminating day \(d_{sub}\). %
For the output's ``spending-transacting'' we thus want to aggregate outputs that represent money put into circulation with respect to the new window \(\wndw + 1\). %
In Algorithm \ref{algo:code_outs_spent_within}, this is illustrated in line 7, where \(\MCircAdj\) is built up reusing \(GetMCirc\) but this time applied to \(t_{\spendingtt}\) and \(\wndw + 1\). %
If an output triggers 
