...
Money in circulation for period $\wndw$ adopting the \ac{wba} ($ \MCircWbWl $), and both the \ac{mca} with the \ac{lifo} rule ($ \MCircMlWl $) and the \ac{fifo} rule ($ \MCircMfWl $).
...
Note, that helper-structures are not necessarily all applied to the calculation of \(\MCirc_{\wndw}[p]\) but might be applied in a shifted fashion. %
...
All inputs are searched for such that are referencing to outputs of coinbase transactions or to outputs generated in transactions generated before the first timestamp of time window \(\wndw\). (compare Section \ref{sec:orig_approach}). %
The values of such inputs \(\val(i)\) are then aggregated to form \(\MCirc\). %
...
In Algorithm \ref{algo:code_outs_spent_within}, this is illustrated by adding value of output XYZ
