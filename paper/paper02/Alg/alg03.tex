...
%
	...
	{%
		...
		$\valO^\sendToOthers(t)  \gets{\sum_{o\in{}\OutT} \val(o)}$              \tcc*{Initialize amount of money sent to third parties with sum of outputs}%
		...
		\eIf(\tcc*[f]{\ac*{wba}: Regarding all outgoing coins ensures all inputs are regarded}%
			...
			$\valO^\sendToOthers(t)  \gets{\valO^\sendToOthers(t) + \sum_{f\in{}\FeeT} \val(f)}$%
			%
	    ...
	    \ForEach {$i \in \Inp'_{t}(\typett)$\label{algo:code_mcirc_for}%
			...
			\If(\tcc*[f]{Input $i$ was generated within $\wndw$ by a coinbase transaction}){%
				...
				$\keyInp \gets {"\addr(i)\_\val(i)"}$                              \tcc*{dictionary/map key for $\Out^{\cbs}_{i}$, supposed to be unique}%
			...
			$ \MCirc(t) \gets{\MCirc(t) + \valIAdd} $                              \tcc*{Update money supply}%
		  	\If{%
		  		...
		  		\If(\tcc*[f]{Use $\valO^\sendToOthers(t)$ as upper cap for  $\MCirc(t)$}){%
		  			...
		  			break                                                          \tcc*{Break foreach-loop on line \ref{algo:code_mcirc_for} and proceed with line \ref{algo:code_mcirc_break}}%
		  		}% 
		  	}% 
		}%
	   	$ \MCirc \gets{\MCirc + \MCirc(t)} $\label{algo:code_mcirc_break}%
	}%
