...
%	\KwData{$ \TxChecked$                                                          \tcc*{Helper to recall all spending transactions linked to same spent tx with already adjusted $\MCirc$}}%
	...
	\ForEach{$o \in \Out_{t}$}{%
		...
		$\TxIndexAdjustment \gets {\TxIndexAdjustment\,\cup\,\{\indextt(t_{\spendingtt})\}}$       \tcc*[f]{Mark $t_\spendingtt$ as already added to some adjustment value \(\MCircAdj\)}\\%
...
%		$\MCircNew \gets{ GetMCirc\bigl(t,\,\typett,\,\wndw+1\bigr)}$\\%
		$\MCircAdj \gets {\MCircAdj\,+\,GetMCirc\bigl(t_{\spendingtt},\,\wndw+1\bigr)}$      \tcc*{Update $\MCircAdj$}%
	}%
%
